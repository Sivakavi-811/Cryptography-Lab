...
def dh_exchange_steps(p, g, private_a=None, private_b=None):
    """Calculate DH steps"""
    steps = []
    
    # 1. Parameter Validation
    is_p_prime = is_prime_simple(p)
    is_g_valid = g < p and g > 1
    
    validations = []
    if not is_p_prime:
        validations.append(f"⚠️ {p} is not a prime number. Diffie-Hellman requires a prime modulus.")
    if not is_g_valid:
        validations.append(f"⚠️ {g} is not a valid generator. It must be between 1 and p-1.")
    
    steps.append({
        'phase': 'Parameter Validation',
        'p': p,
        'g': g,
        'is_p_prime': is_p_prime,
        'validations': validations
    })
    
    # 2. Primitive Root Check
    is_g_primitive = is_primitive_root(g, p)
    note = f"{g} is a primitive root of {p} ✓" if is_g_primitive else f"{g} is NOT a primitive root of {p}. The exchange will still work mathematically, but it's less secure as it may not cover all values in the group."
    
    steps.append({
        'phase': 'Primitive Root Check',
        'p': p,
        'g': g,
        'primitive_root': get_first_primitive_root(p),
        'note': note
    })
    
    # Generate random keys if not provided
    if private_a is None: private_a = random.randint(2, p-1)
    if private_b is None: private_b = random.randint(2, p-1)
    
    # 3. A Key Generation
    A = mod_exp_simple(g, private_a, p)
    steps.append({
        'phase': 'A Key Generation',
        'private_key': private_a,
        'public_key': A,
        'steps': [
            {'step': 1, 'operation': 'Choose Private a', 'details': f'a = {private_a}'},
            {'step': 2, 'operation': 'Compute Public A', 'details': f'A = {g}^{private_a} mod {p} = {A}'}
        ]
    })
    
    # 4. B Key Generation
    B = mod_exp_simple(g, private_b, p)
    steps.append({
        'phase': 'B Key Generation',
        'private_key': private_b,
        'public_key': B,
        'steps': [
            {'step': 1, 'operation': 'Choose Private b', 'details': f'b = {private_b}'},
            {'step': 2, 'operation': 'Compute Public B', 'details': f'B = {g}^{private_b} mod {p} = {B}'}
        ]
    })
    ...
    steps.append({
        ...
        'A_public_key': A,
        'B_public_key': B,
        'exchange': f"A sends {A} to B, and B sends {B} to A."
    })
    
    # 6. Shared Secret Calculation
    K_A = mod_exp_simple(B, private_a, p)
    steps.append({
        'phase': 'A Shared Secret',
        'secret': K_A,
        'steps': [{
            'operation': 'Calculate A Shared Secret',
            'formula': 'K = B^a mod p',
            'calculation': f'K = {B}^{private_a} mod {p}',
            'result': K_A
        }]
    })
    
    K_B = mod_exp_simple(A, private_b, p)
    steps.append({
        'phase': 'B Shared Secret',
        'secret': K_B,
        'steps': [{
            'operation': 'Calculate B Shared Secret',
            'formula': 'K = A^b mod p',
            'calculation': f'K = {A}^{private_b} mod {p}',
            'result': K_B
        }]
    })
    ...
    steps.append({
        ...
        'B_secret': K_B,
        'match': match,
        'result': f"{'Success! Both keys are equal.' if match else 'Failure! Keys do not match.'}"
    })
...
@app.route("/diffiehellman", methods=["GET", "POST"])
def diffiehellman():
    operation = request.form.get("operation", "exchange")
    p_val, g_val, private_a_val, private_b_val = None, None, None, None
    steps = []
    error = None
    ...
    if request.method == "POST":
        if operation == "demo":
            ...
            steps = [{
                ...
                'p': p, 'g': g, 'a': private_a, 'b': private_b,
                'A': A, 'B': B, 'K_A': K_A, 'K_B': K_B,
                'match': (K_A == K_B)
            }]
        elif operation == "validate":
            try:
                p_val_str = request.form.get("p", "0")
                g_val_str = request.form.get("g", "0")
                p_val = int(p_val_str) if p_val_str else 0
                g_val = int(g_val_str) if g_val_str else 0
                
                if p_val <= 1:
                    error = "Prime p must be greater than 1."
                else:
                    is_p_prime = is_prime_simple(p_val)
                    prim_root = get_first_primitive_root(p_val)
                    steps = [{
                        'phase': 'Validation Result',
                        'p': p_val, 'g': g_val,
                        'is_prime': is_p_prime,
                        'primitive_root': prim_root,
                        'valid': (is_p_prime and g_val < p_val and g_val > 1)
                    }]
            except ValueError:
                error = "Please provide valid integer parameters."
        else: # exchange
            try:
                p_val_str = request.form.get("p", "0")
                g_val_str = request.form.get("g", "0")
                ...
                a_str = request.form.get("private_a", "")
                b_str = request.form.get("private_b", "")
                
                private_a = int(a_str) if a_str else None
                private_b = int(b_str) if b_str else None
                
                if p_val <= 1 or g_val <= 0:
                    error = "Parameters p and g must be positive integers."
                else:
                    steps = dh_exchange_steps(p_val, g_val, private_a, private_b)
            except ValueError:
                error = "Please provide valid integer parameters."
                
    return render_template("index.html", page="diffiehellman", 
                           operation=operation, steps=steps, error=error,
